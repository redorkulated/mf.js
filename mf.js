...
var $Ajax= {
	...
	Send: function(R,functionCallback,content){
		if(this._RequestStack[R]){
			if(!functionCallback){
				this._RequestStack[R].Error="$Ajax.send callback function not found";
			...
			if(this._RequestStack[R].Method == "HEAD"){ //Head request, will return no content just headers/status
				this._RequestStack[R].Core.onreadystatechange= function(){
					if($Ajax.CoreRequest(R).readyState == 4){
	...
	Header: function (R,H,V){
		...
		if(this._RequestStack[R].State != this.States.Received && this._RequestStack[R].State != this.States.Open){ return false; }
		if(H){
	...
	ResponseXML: function (R){ return (this._RequestStack[R]?this._RequestStack[R].Core.responseXML:false); },
	ResponseText: function (R){ return (this._RequestStack[R]?this._RequestStack[R].Core.responseText:false); },
	Status: function (R){ return (this._RequestStack[R]?this._RequestStack[R].Core.status:false); },
